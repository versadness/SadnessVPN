...
![Release: 3.1.3 Beta](https://img.shields.io/badge/release-3.1.3_Beta-8b5cf6)
...
### [Скачать полный Setup.exe для Windows x64 · около 166 МиБ](https://github.com/versadness/SadnessVPN/releases/download/v3.1.3-beta/SadnessVPN-3.1.3-Setup-x64.exe)

[Описание версии 3.1.3](https://github.com/versadness/SadnessVPN/releases/tag/v3.1.3-beta) · [Все версии](https://github.com/versadness/SadnessVPN/releases)

Файл **`SadnessVPN-3.1.3-Setup-x64.exe`** — однофайловый установщик с иконкой приложения и полным MSI внутри. Python, Electron и VPN-ядра уже включены; WebView2 отдельно скачивать не требуется.

Архивы **Source code** на странице релиза — снимки репозитория дистрибутивов, а не установщики клиента.

## Что нового в 3.1.3

- Исправлено подключение к обычному WireGuard (не AmneziaWG): раньше ядро запускалось без туннеля.
- Исправлено автообновление: новая версия больше не откатывается после установки.
- Автозапуск Windows снова открывает приложение со значком в трее; «Тихий запуск» и автоподключение работают.
...
- Zapret исключает адреса WireGuard- и AmneziaWG-серверов и возвращает настройку TCP timestamps после остановки.
- Повреждённый файл настроек больше не сбрасывает профиль и ID устройства.
- Плагины Shadowsocks и HTTP-маскировка TCP сохраняются при импорте, а не отбрасываются молча.
- В настройках DNS появились Quad9 и «Свой DNS».

С 3.0.9 также вошли изменения 3.0.10–3.0.12, которые отдельно не публиковались: единственное ядро Mihomo, поддержка VMess, проверка сервера настоящим запросом, очередь обновления подписок, охват Zapret и предупреждение о сервере, переставшем принимать соединения.

Подробности и проведённые проверки перечислены на [странице релиза 3.1.3](https://github.com/versadness/SadnessVPN/releases/tag/v3.1.3-beta).
...
3. Скачайте `SadnessVPN-3.1.3-Setup-x64.exe` только из Assets официального релиза.
...
- Файл: `SadnessVPN-3.1.3-Setup-x64.exe`
- Версия: **3.1.3.0**
- Размер: **174 376 471 байт**
- SHA256: `b9d74a5bf8c15780cbe92e60e684809f8f470fd54d07111bf02bbced8fd4007d`

Проверка в PowerShell:

```powershell
Get-FileHash -LiteralPath '.\SadnessVPN-3.1.3-Setup-x64.exe' -Algorithm SHA256
```
